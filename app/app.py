...
tabs = st.tabs(["Chat", "Metrics"])
chat_tab, metrics_tab = tabs
...
from src.bq_query_metrics import query_df

with metrics_tab:
    st.subheader("Day 10 Experiment Metrics (BigQuery)")

    sql = """
    SELECT
      variant,
      COUNT(*) AS n,
      AVG(CAST(refusal AS INT64)) AS refusal_rate,
      AVG(CAST((NOT refusal) AND (n_citations > 0) AS INT64)) AS citation_rate_nonrefusal,
      AVG(diversity_unique_doc_ids) AS avg_diversity,
      APPROX_QUANTILES(latency_ms, 100)[OFFSET(95)] AS p95_latency_ms,
      AVG(CAST(answerable AS INT64)) AS answerable_frac,
      AVG(CAST(refusal = expected_refusal AS INT64)) AS expected_refusal_accuracy
    FROM `rag_eval_lab.eval_outputs_day10`
    GROUP BY variant
    ORDER BY variant;
    """
    ...
    # Quick “who wins” hint
    if not df.empty:
        best = df.sort_values(by="expected_refusal_accuracy", ascending=False).iloc[0]
        st.success(
            f"Best expected_refusal_accuracy: {best['variant']} "
            f"({best['expected_refusal_accuracy']:.2f})"
        )